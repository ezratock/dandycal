...
// ---------- Core functions: image/text → structured JSON → URL ----------
...
/**
 * Take event text content, extract calendar fields via Gemini structured output,
 * and build a Google Calendar template URL.
 *
 * @param {string} textContent
 *   Text content extracted from a DOM element containing event information.
 * @param {string} apiKey
 *   Gemini API key.
 * @returns {Promise<{ event: any, url: string }>}
 */
export async function createCalendarUrlFromText(textContent, apiKey) {
	if (!apiKey) {
	...
	const prompt = [
		"You are given text extracted from a webpage containing calendar event details.",
		...
		"If no year or month are specified assume the event is upcoming relative to current date.",
		"Do not hallucinate dates or details not present in the text.",
		"",
		...
		"    is last-day + 1.",
		"- Prefer the timezone mentioned in the text for 'ctz'; otherwise guess",
